...
<div class="flex h-[calc(100vh-140px)] bg-slate-50">
    {{-- Left Icon Navbar --}}
    ...
    {{-- Sidebar Tools Panel --}}
    <div x-show="sidebarOpen" x-transition class="w-72 bg-white border-r border-slate-200 shadow-sm flex flex-col flex-shrink-0 relative z-20">
        <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <h3 class="font-black text-slate-800 text-sm uppercase tracking-widest" x-text="activeTab === 'templates' ? 'Template Desain' : (activeTab === 'upload' ? 'Upload Gambar' : (activeTab === 'stickers' ? 'Cari Stiker' : 'Tambahkan Teks'))"></h3>
    ...
    <div class="flex-1 flex flex-col relative bg-slate-100 overflow-hidden">
        {{-- Step Label --}}
        <div class="h-14 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 shadow-sm z-10 w-full flex-shrink-0">
            <div class="flex items-center gap-4">
                <span class="text-sm font-black text-slate-800 uppercase tracking-widest" x-text="currentStep === 2 ? 'Desain Bagian Depan (Wajib)' : 'Desain Bagian Belakang (Opsional)'"></span>
                <span class="text-xs font-bold px-3 py-1 rounded-full" :class="currentStep === 2 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'" x-text="currentStep === 2 ? 'STEP 2' : 'STEP 3'"></span>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-auto flex justify-center items-center py-8 relative bg-slate-50">
            <div class="relative shadow-2xl rounded-xl overflow-hidden pointer-events-auto flex items-center justify-center bg-slate-100" id="mockupContainer" style="width: 480px; height: 600px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.15);">
                ...
                <div class="absolute top-4 left-4 z-30 pointer-events-none">
                    <span class="bg-indigo-600/90 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-lg border border-indigo-400/50 uppercase tracking-widest backdrop-blur-sm" x-text="activeSide === 'front' ? 'Bagian Depan' : (activeSide === 'back' ? 'Bagian Belakang' : activeSide.toUpperCase())">Bagian Depan</span>
                </div>
    ...
    <div id="editorControls" class="w-80 bg-white border-l border-slate-200 shadow-sm flex flex-col flex-shrink-0 z-30 hidden overflow-y-auto custom-scrollbar">
        ...
        <div class="p-6 space-y-8">
            ...
            <div id="objectSablonSizeControl" class="space-y-3 pb-6 border-b border-slate-100">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <button type="button" id="btnSizeA5" onclick="setObjectSablonSize('a5')" class="size-btn flex flex-col items-center justify-center p-2 rounded-xl border border-slate-200 bg-slate-50 hover:border-red-400 transition cursor-pointer">
                        <span class="text-xs font-black text-slate-800">A5 Logo</span>
                        <span class="text-[9px] text-slate-400 mt-0.5">10x10 cm</span>
                        <span class="text-[10px] font-bold text-red-600 mt-1">Rp 10k</span>
                    </button>
                    <button type="button" id="btnSizeA4" onclick="setObjectSablonSize('a4')" class="size-btn flex flex-col items-center justify-center p-2 rounded-xl border border-slate-200 bg-slate-50 hover:border-red-400 transition cursor-pointer">
                        <span class="text-xs font-black text-slate-800">A4</span>
                        <span class="text-[9px] text-slate-400 mt-0.5">20x25 cm</span>
                        <span class="text-[10px] font-bold text-red-600 mt-1">Rp 25k</span>
                    </button>
                    <button type="button" id="btnSizeA3" onclick="setObjectSablonSize('a3')" class="size-btn flex flex-col items-center justify-center p-2 rounded-xl border border-slate-200 bg-slate-50 hover:border-red-400 transition cursor-pointer">
                        <span class="text-xs font-black text-slate-800">A3</span>
                        <span class="text-[9px] text-slate-400 mt-0.5">25x35 cm</span>
                        <span class="text-[10px] font-bold text-red-600 mt-1">Rp 35k</span>
                    </button>
                </div>
            </div>
            ...
            <div id="imageControls" class="hidden flex-col gap-4">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Hapus Latar</p>
                <div class="flex items-center gap-4">
                    <div class="flex-1"><label class="text-[9px] font-black text-slate-400 uppercase block mb-1">Warna Target</label><input type="color" id="removeColorTarget" class="w-full h-8 p-0.5 border border-slate-200 rounded-lg cursor-pointer bg-white" value="#ffffff"></div>
                ...
                <div class="flex gap-2">
                    <button id="detectBgColorBtn" class="flex-1 bg-slate-100 text-slate-600 text-xs font-bold py-3 rounded-xl hover:bg-slate-200 transition border border-slate-200 shadow-sm">🔍 Auto Detect</button>
                    <button id="removeBgBtn" class="flex-1 bg-sky-50 border border-sky-100 text-sky-600 text-xs font-black py-3 rounded-xl hover:bg-sky-100 transition uppercase tracking-widest">✨ EKSEKUSI</button>
                </div>
            ...
            <div class="pt-6 border-t border-slate-100">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button id="bringForwardBtn" class="flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold py-3 px-4 rounded-xl border border-slate-200 transition text-xs">↑ Ke Depan</button>
                    <button id="sendBackwardBtn" class="flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold py-3 px-4 rounded-xl border border-slate-200 transition text-xs">↓ Ke Belakang</button>
                </div>
            </div>
            {{-- Delete --}}
